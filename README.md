...
> By **BunnY** · 搭配 Bunnyhole Lab 食用 · v8.0.11
...
图片模式下输入框变为单行备注，可不填；点击「导入图片」后，图片会上传到 carrot 后端缓存并自动插入酒馆输入框。聊天前端只显示图片，不显示备注；使用支持视觉输入的模型时，AI 会同时收到图片。
...
├── image-upload.js        # v8.0.11 carrot 图片上传
├── link-vision.js         # v8.0.11 链接封面 / carrot 图片多模态注入
├── stickers.js            # 表情包逻辑
