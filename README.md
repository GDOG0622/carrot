...
> By **BunnY** · 搭配 Bunnyhole Lab 食用 · v8.0.24
...
|------|----------|------|
| 纯文本 | `"内容"` | `"你好"` |
| 图片 | `<carrot-image src="...">备注</carrot-image>`；未导入图片时为 `[描述.jpg]` | 导入图片后自动插入 |
| 视频 | `[描述.mp4]` | `[猫咪.mp4]` |
| 音乐 | `[描述.mp3]` | `[晚风.mp3]` |
| 钱包/转账 | `[平台|金额|留言]` | `[微信|50|奶茶]` |

图片模式下输入框变为单行备注，可不填；点击「导入图片」只会先选择图片，点击「插入」时才上传到 carrot 后端缓存并插入酒馆输入框。不导入图片、只填描述时，点击「插入」会直接以 `[描述.jpg]` 纯文本形式插入。聊天前端只显示图片，不显示备注；使用支持视觉输入的模型时，AI 会同时收到图片。上传图最多保留 10 个，旧图自动清理。

底部麦克风用于语音输入，输出格式：`=录音时长|识别文本=`。

面板左下角齿轮打开设置中心，含「主题 / 头像 / 气泡 / 同步」四个标签；「同步」用于导出/导入扩展全部配置。
...
- 可在 carrot 设置面板 → API → 链接解析中开启/关闭“把链接封面作为图片发给视觉模型”；默认开启，使用支持视觉输入的模型时，AI 可以读到后端缓存的链接封面图
...
6. 可与「后台时弹出系统通知」同时开启，自动去重：本地通知优先（即时），本地弹不出来（如 iOS）时才走后端推送兜底，不会重复弹两条
