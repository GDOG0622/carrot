...
> By **BunnY** · 搭配 Bunnyhole Lab 食用 · v8.0.20
...
|------|----------|------|
| 纯文本 | `"内容"` | `"你好"` |
| 图片 | `<carrot-image src="...">备注</carrot-image>` | 导入图片后自动插入 |
| 视频 | `[描述.mp4]` | `[猫咪.mp4]` |
| 音乐 | `[描述.mp3]` | `[晚风.mp3]` |
| 钱包/转账 | `[平台|金额|留言]` | `[微信|50|奶茶]` |

图片模式下输入框变为单行备注，可不填；点击「导入图片」只会先选择图片，点击「插入」时才上传到 carrot 后端缓存并插入酒馆输入框。聊天前端只显示图片，不显示备注；使用支持视觉输入的模型时，AI 会同时收到图片。上传图最多保留 10 个，旧图自动清理。
...
├── image-upload.js        # v8.0.20 carrot 图片上传
├── link-vision.js         # v8.0.20 链接封面 / carrot 图片多模态注入
├── stickers.js            # 表情包逻辑
